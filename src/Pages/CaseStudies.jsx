import { BookOpen, Users, BarChart2, Clock } from 'lucide-react';
      ...
      <div className="w-5/6 mx-auto py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {caseStudies.map((study) => (
            <div 
              key={study.id}
              className="bg-white/5 rounded-lg border border-gray-400 hover:border-white transition-all duration-300 backdrop-blur-sm p-8"
            >
              <div className="flex items-center justify-center h-16 w-16 rounded-full bg-indigo-600/30 mx-auto mb-6">
                {study.icon}
              </div>
              <h3 className="text-2xl font-bold text-white text-center mb-4">{study.title}</h3>
              <p className="text-gray-300 text-center mb-6">{study.description}</p>
